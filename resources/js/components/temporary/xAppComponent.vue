<template>
    ...
    <div v-for="architect in architects" :key="architect">
        <CreateArchitectForm :architect="architect"/>
        <p :id="'clicked' + architect.id" @click="clicker"> {{ architect.name }} {{ architect.surname }}</p>
    </div>
</template>

<script>
import Modal from './Modal.vue'
import CreateArchitectForm from './CreateArchitectForm.vue'

    export default {
        name: "App",
        components: { Modal, CreateArchitectForm },
        data() {
            return {
                name: '',
                architects: [],
